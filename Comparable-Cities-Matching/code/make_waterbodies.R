make_waterbodies <- function(city_shapes, utmcrs) {
    #' @title Preparing waterbodies
    #' 
    #' @description This function aggregates the waterbodies on the
    ...
    #' @return Summarised waterbodies on city-level
    ...
    waterbodies_sum <- waterbodies |>
        # add area of each waterbody
        dplyr::mutate(
            water_area_sqkm = as.numeric(
                sf::st_area(geometry)
            ) / 1000000
        ) |>
        sf::st_drop_geometry() |>
        ...
        dplyr::summarise(
            total_water_area_sqkm = sum(water_area_sqkm)
        )

    #----------------------------------------------
    # summarise by city and year

    waterbodies_sum_total <- waterbodies_sum |>
        dplyr::group_by(city_name, year) |>
        dplyr::summarise(
            total_water_area_sqkm = sum(total_water_area_sqkm)
        ) |>
        ...
        # calculate the relative area of the waterbodies
        # relative to the city size
        dplyr::mutate(
            rel_total_water_area = total_water_area_sqkm /
                city_size_sqkm
        )
    ...
    cols <- sample(
        cols,
        length(unique(waterbodies$used_tag)),
        replace = FALSE
    )

    # define labels
    labs <- stringr::str_to_title(unique(waterbodies$used_tag))
    ...
    for (ct in cities) {
        ...
        filename <- paste0("waterbodies_2020_", ct, ".png")
        ggsave(
            plot = map,
            file.path(
                output_path,
    ...
    data.table::fwrite(
        waterbodies_sum_total,
        file.path(
            data_path,
            "waterbodies/waterbodies_cities.csv"
        )
    )

    #----------------------------------------------
    # return
    return(waterbodies_sum_total)
}